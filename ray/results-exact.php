...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.filterPanelManager) {
        console.log('Filter panel integrated with results-exact.php');
    } else {
        // Fallback: check if filter panel exists and initialize
        setTimeout(() => {
            if (window.filterPanelManager) {
                console.log('Filter panel integrated with results-exact.php (delayed)');
            }
        }, 100);
    }
    
    // Listen for filter panel events if neede
    document.addEventListener('filterPanelOpened', function() {
        console.log('Filter panel opened on results page');
    });
    
    document.addEventListener('filterPanelClosed', function() {
        console.log('Filter panel closed on results page');
    });
});
</script>
